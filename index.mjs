 ...
 *
 * This is the ES Module version of chain-schema-validator.
 */
...
// --- Utilities ---
export const ref = (key) => ({ isRef: true, key });
...
class Validator {
    ...
    async _validate(value, originalObj) {
        ...
        for (const transformer of this._transformers) { currentValue = transformer(currentValue); }
        for (const rule of this._rules) {
            let isValid = (rule.type === 'customAsync') ? await rule.validate(currentValue, originalObj) : rule.validate(currentValue, originalObj);
